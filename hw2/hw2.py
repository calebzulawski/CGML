...
class Model():
    ...
    def build_model(self):
        ...
        self.out = tf.matmul(layer_2, wo) + bo

    def train(self, xs, ys, cs):
        two = tf.constant(2, dtype=tf.float32)
        cost = tf.reduce_sum(tf.divide(tf.squared_difference(self.y, self.out), two))
        ...
        for epoch in range(self.n_epochs):
            for x, y, c in zip(xs, ys, cs):
                ...
                l = np.expand_dims(np.asarray([c]), axis=1)
                self.sess.run(optimizer, feed_dict={self.x: v, self.y: l})

    def predict(self, x, y):
        v = np.expand_dims(np.asarray([x, y]), axis=1)
        return self.sess.run(self.out, feed_dict={self.x: v})[0][0] > 0.5
